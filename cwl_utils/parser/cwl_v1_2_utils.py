...
def _input_type_schema_to_cwl_v1_2_input_type_schema(
...
def input_type_schema_to_cwl_v1_2_input_type_schema(
    input_type: cwl_utils.parser.InputTypeSchemas, loading_options: cwl.LoadingOptions
) -> InputTypeSchemas:
    if is_sequence(input_type):
        return [
            _input_type_schema_to_cwl_v1_2_input_type_schema(
                input_type_item, loading_options
            )
            for input_type_item in input_type
        ]
    return _input_type_schema_to_cwl_v1_2_input_type_schema(input_type, loading_options)


def _output_type_schema_to_cwl_v1_2_output_type_schema(
        ...
        for step_input in step_run.inputs:
            if step_input.id.split("#")[-1] == in_.id.split("#")[-1]:
                if step_input.type_ is not None:
                    step_input_type = input_type_schema_to_cwl_v1_2_input_type_schema(
            ...
            if (
                ...
                if step.scatter is not None:
                    if output_type is not None:
                        output_type_type = (
                            output_type_schema_to_cwl_v1_2_output_type_schema(
...
def type_for_source(
    ...
    if pickValue is not None:
        if isinstance(final_type, cwl.ArraySchema):
            if pickValue in ("first_non_null", "the_only_non_null"):
                return final_type.items
    return final_type
