...
def expand_stream_shortcuts(process: cwl.CommandLineTool) -> cwl.CommandLineTool:
    ...
    for index, output in enumerate(process.outputs):
        if output.type_ == "stdout":  # TODO: add 'stdin' for CWL v1.1
            ...
            output.outputBinding = cwl.CommandOutputBinding(glob=stdout_path)
    if result:
        return result
    return process
...
def _plain_output_schema_to_plain_input_schema(
    input_type: BasicOutputTypeSchemas,
...
def plain_output_schema_to_plain_input_schema(
    input_type: OutputTypeSchemas,
) -> InputTypeSchemas:
    if isinstance(input_type, Sequence):
        return [
            _plain_output_schema_to_plain_input_schema(input_type_item)
            for input_type_item in input_type
        ]
    return _plain_output_schema_to_plain_input_schema(input_type)
...
def parameters_to_workflow_input_paramaters(
    parameters: Sequence[
        cwl.WorkflowInputParameter
        | cwl.CommandInputParameter
        | cwl.CommandOutputParameter
    ],
) -> Sequence[cwl.WorkflowInputParameter]:
    return [
        parameter_to_workflow_input_paramater(parameter) for parameter in parameters
    ]
...
def generate_etool_from_expr(
    ...
    if not no_inputs:
        ...
        inputs.append(
            cwl.WorkflowInputParameter(
                id="self",
                label=self_type.label if not isinstance(self_type, Sequence) else None,
                secondaryFiles=(
                    self_type.secondaryFiles
                    if not isinstance(self_type, Sequence)
                    else None
                ),
                streamable=(
                    self_type.streamable
                    if not isinstance(self_type, Sequence)
                    else None
                ),
                doc=self_type.doc if not isinstance(self_type, Sequence) else None,
                format=(
                    self_type.format if not isinstance(self_type, Sequence) else None
                ),
                type_=new_type,
                extension_fields=(
    ...
    for index, param in enumerate(workflow.inputs):
        with SourceLine(workflow.inputs, index, WorkflowException):
            ...
            if param.secondaryFiles:
                if hasattr(param.secondaryFiles, "pattern") and has_expression(
                    param.secondaryFiles.pattern
                ):
                    raise SourceLine(
                        param.loadingOptions.original_doc,
                        "secondaryFiles",
                    ...
                    for index2, entry in enumerate(param.secondaryFiles):
                        if has_expression(entry.pattern):
                            raise SourceLine(
                                param.loadingOptions.original_doc,
                                index2,
                                raise_type=WorkflowException,
        ...
        with SourceLine(workflow.outputs, index3, WorkflowException):
            if param2.pickValue:
                ...
                target = cwl.WorkflowInputParameter(
                    id="", type_=plain_output_schema_to_plain_input_schema(target_type)
                )
                assert param2.outputSource is not None
                if not isinstance(param2.outputSource, Sequence):
                ...
                if isinstance(source_type_items, cwl.ArraySchema):
                    if isinstance(source_type_items.items, Sequence):
                        if "null" not in source_type_items.items:
                            new_source_type_items_items = list(source_type_items.items)
                            new_source_type_items_items.append("null")
                            source_type_items.items = new_source_type_items_items
                    elif source_type_items.items != "null":
                        source_type_items.items = ["null", source_type_items.items]
                elif isinstance(source_type_items, Sequence):
                    if "null" not in source_type_items:
                        new_source_type_items = list(source_type_items)
                        new_source_type_items.append("null")
                        source_type_items = new_source_type_items
            ...
            if outp.outputBinding.outputEval and not skip_command_line2:
                ...
                if expression:
                    ...
                    if isinstance(outp, cwl.CommandOutputParameter):
                        target = parameter_to_workflow_input_paramater(outp)
                    else:
                        target = outp
...
def generate_etool_from_expr2(
    expr: str,
    target: cwl.WorkflowInputParameter | cwl.CommandInputParameter,
    inputs: Sequence[
        cwl.WorkflowInputParameter
        | cwl.CommandInputParameter
        | cwl.CommandOutputParameter
        ...
        if inp.valueFrom:
            if not inp.source:
                ...
                for source in inp.source:
                    if not step.scatter:
                        ...
                        if isinstance(scattered_source_type, Sequence):
            ...
            if expression:
                ...
                source_type: (
                    None
                    | MutableSequence[
                        cwl.WorkflowInputParameter | cwl.CommandOutputParameter
                    ]
                    | cwl.WorkflowInputParameter
                    | cwl.CommandOutputParameter
                ) = None
                if inp.source:
                    ...
                    for source in inp.source:
                        ...
                        if isinstance(temp_type, Sequence):
        ...
        if inp.source and inp_id != except_in_id:
            ...
            if isinstance(param, Sequence):
                for p in param:
                    ...
                    new_param = parameter_to_workflow_input_paramater(p)
                    new_param.id = inp_id
                ...
                new_param = parameter_to_workflow_input_paramater(param)
                new_param.id = inp_id
                new_param.type_ = clean_type_ids(new_param.type_)
                params.append(new_param)
    return params
